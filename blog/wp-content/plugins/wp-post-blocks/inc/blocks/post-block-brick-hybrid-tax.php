...
if( !class_exists( 'Block_Brick_Hybrid_Tax') ){

	if( !class_exists( 'Block_Brick' ) ){
		require_once Plugin::get_dir() . 'inc/post-block-brick.php';
	}

	class Block_Brick_Hybrid_Tax extends Block_Brick{

		static $id 			= 'pbs-brick-hybrid-tax';
		static $react 		= 'post_block_brick_hybrid_tax';
		static $class 		= 'pbs pbs-brick pbs-brick-hybrid-tax';
		static $name 		= 'Bricks - Hybrid Taxonomy';
					...
					<div class="brick brick-o brick-lg col__md-1_2 brick-tax vtext-top">
						<div class="brick-i">
							<div class="brick-c">
								...
								<div class="brick-c-i">
									<div class="brick-e-h">
										<h2 class="brick-e-t xs__h3 sm__h3 md__h2 lg__h1"><?php echo wp_sprintf( '<a href="%s" title="%s">%s</a>', esc_url( get_term_link( $term ) ), esc_attr( wp_sprintf( __('View all posts in %s', 'wp-post-blocks'), $term->name ) ), esc_attr( $term->name ) );?></h2>				
									</div>
		...
		public function render_template( $settings = array(), $misc = array() ){
			...
			$class = 'brick-sm col__sm-1_2 col__md-1_4';
