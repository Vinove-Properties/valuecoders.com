...
/**
 * Counter, title, Post thumbnail
 * @since 1.0
 */

if ( ! defined( 'ABSPATH' ) ) exit; 

if( !class_exists( 'Post_Template_Counter_Thumb') ){

	if( !class_exists( 'Post_Template' ) ){
		require_once Plugin::get_dir() . 'inc/post-template.php';
	}

	class Post_Template_Counter_Thumb extends Post_Template{
		static $thumb_size = 'medium';
		static $uid			= 'pbs-module-counter-thumb';
		/**
		 * Render the block
		 */
		...
		static function render( $settings = array(), $misc = array() ){
			...
			$template_class .= ' ' . 'pbs-module-thumb-right pbs-module-counter';
