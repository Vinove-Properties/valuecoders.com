...
class RevisionaryLicenseSettings {
        ...
        <tr>
            <td scope="row" colspan="2">
                ...
                if ($expired) {
                    ...
                    $is_err = true;
                    $msg = sprintf(
                        esc_html__('Your license key has expired. For continued priority support, <a href="%s">please renew</a>.', 'revisionary'),
                    ...
                    if ($expire_days == 1) {
                        $msg = sprintf(
                            esc_html__('Your license key will expire today. For updates and priority support, <a href="%s">please renew</a>.', 'revisionary'),
                    ...
                    $class = 'activating';
                    $msg = sprintf(
                        esc_html__('For updates to PublishPress Revisions Pro, activate your %sPublishPress license key%s.', 'revisionary'),
                ...
                <?php if ($expired && (!empty($key))) : ?>
                    <span class="pp-key-expired"><?php esc_html_e("Key Expired", 'revisionary') ?></span>
                    ...
                    <button type="button" id="activation-button" name="activation-button"
                            class="button-secondary"><?php esc_html_e('Deactivate Key', 'revisionary'); ?></button>
                <?php else : ?>
                    <div class="pp-key-label" style="float:left">
                        ...
                        <button type="button" id="activation-button" name="activation-button"
                                class="button-secondary"><?php if (!$activated) esc_html_e('Activate Key', 'revisionary'); else esc_html_e('Deactivate Key', 'revisionary'); ?></button>
                <?php endif; ?>
                    ...
                    <div class="pp-key-refresh" style="display:inline">
                        &bull;&nbsp;&nbsp;<a href="https://publishpress.com/checkout/purchase-history/"
                                                    target="_blank"><?php esc_html_e('review your account info', 'revisionary'); ?></a>
                    </div>
                </div>
                ...
                <?php if ($activated) : ?>
                    <?php if ($expired) : ?>
                        <div class="pp-key-hint-expired">
                            <span class="pp-key-expired pp-key-warning"> <?php esc_html_e('note: Renewal does not require deactivation. If you do deactivate, re-entry of the license key will be required.', 'revisionary'); ?></span>
                        </div>
                    <?php elseif (revisionary()->getOption('display_hints')) : ?>
                        <div class="pp-key-hint">
                        <span class="rs-subtext"> <?php esc_html_e('note: If you deactive, re-entry of the license key will be required for re-activation.', 'revisionary'); ?></span>
                    <?php endif; ?>
                    </div>
        ...
        ?>
            <tr>
                <td>
                    ...
                    <div class="pp-key-label " style="float:left">
                        <?php printf(
                            esc_html__('%1$s PublishPress Revisions Pro %2$s %3$s', 'revisionary'), 
                            sprintf(esc_html__('%1$sInstalled Version:%2$s', 'revisionary'), '<span class="pp-key-inactive">', '</span>'), 
                            esc_html(PUBLISHPRESS_REVISIONS_VERSION), 
                            ''
                        ); 
                        
                        if (!empty($do_info_link)) {
                            echo "&nbsp;<span class='update-message'> &bull;&nbsp;&nbsp;<a href='" . esc_url($info_url) . "' class='thickbox'>"
                                . sprintf(esc_html__('view %s&nbsp;details', 'revisionary'), esc_html($wp_plugin_updates->response[plugin_basename(REVISIONARY_FILE)]->new_version))
                                . '</a></span>';
                        }
                        ...
                        &nbsp;&nbsp;&bull;&nbsp;&nbsp;<a href="<?php echo esc_url(add_query_arg('rvy_refresh_updates', 1, $uri));?>"><?php esc_html_e('update check / install', 'revisionary'); ?></a>
    ...
    private function footer_js($activated, $expired)
    {
        $vars = [
            ...
            'activateCaption' => esc_html__('Activate Key', 'revisionary'),
            'deactivateCaption' => esc_html__('Deactivate Key', 'revisionary'),
            'connectingCaption' => esc_html__('Connecting to publishpress.com server...', 'revisionary'),
            'noConnectCaption' => esc_html__('The request could not be processed due to a connection failure.', 'revisionary'),
            'noEntryCaption' => esc_html__('Please enter the license key shown on your order receipt.', 'revisionary'),
            'errCaption' => esc_html__('An unidentified error occurred.', 'revisionary'),
            'keyStatus' => wp_json_encode([
                'deactivated' => esc_html__('The key has been deactivated.', 'revisionary'),
                'valid' => esc_html__('The key has been activated.', 'revisionary'),
                'expired' => esc_html__('The key has expired.', 'revisionary'),
                'invalid' => esc_html__('The key is invalid.', 'revisionary'),
                '-100' => esc_html__('An unknown activation error occurred.', 'revisionary'),
                '-101' => esc_html__('The key provided is not valid. Please double-check your entry.', 'revisionary'),
                '-102' => esc_html__('This site is not valid to activate the key.', 'revisionary'),
                '-103' => esc_html__('The key provided could not be validated by publishpress.com.', 'revisionary'),
                '-104' => esc_html__('The key provided is already active on another site.', 'revisionary'),
                '-105' => esc_html__('The key has already been activated on the allowed number of sites.', 'revisionary'),
                '-200' => esc_html__('An unknown deactivation error occurred.', 'revisionary'),
                '-201' => esc_html__('Unable to deactivate because the provided key is not valid.', 'revisionary'),
                '-202' => esc_html__('This site is not valid to deactivate the key.', 'revisionary'),
                '-203' => esc_html__('The key provided could not be validated by publishpress.com.', 'revisionary'),
                '-204' => esc_html__('The key provided is not active on the specified site.', 'revisionary'),
            ]),
            ...
            'activationHelp' => sprintf(esc_html__('If this is incorrect, <a href="%s">request activation help</a>.', 'revisionary'), 'https://publishpress.com/contact/'),
            'supportOptChanged' => esc_html__('Please save settings before uploading site configuration.', 'revisionary'),
        ];
