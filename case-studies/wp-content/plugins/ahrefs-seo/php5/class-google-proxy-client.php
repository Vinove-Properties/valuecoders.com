...
class Google_Proxy_Client extends Google_Client {
	 ...
	 * @return string
	 */
	public static function get_auth_root() {
	 ...
	 * @return string
	 */
	public function load_sites_or_register_url_from_proxy( $sites_list = false, array $scopes = null ) {
		$error = null;
		$body  = array_merge(
			[
				'client_id'     => $this->getClientId(),
		...
		if ( ! $sites_list || '' === $this->getClientId() || '' === $this->getClientSecret() ) {
			// add scope for non sites list request or unauthorized request.
			$scopes = isset( $scopes ) ? $scopes : $this->getScopes();
			sort( $scopes );
	 ...
	 * @return array
	 */
	protected function get_site_details() {
		return [
