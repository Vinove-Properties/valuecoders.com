...
abstract class Ahrefs_Seo_Screen {
	...
	public function process_post_data() : void {
		...
		if ( isset( $_GET['action'] ) && ( 'google_oauth' === $_GET['action'] ) ) {
			...
			$query = is_null( $error ) ? [] : [ 'error' => $error ];
			if ( Ahrefs_Seo::get()->initialized_get() ) {
				$query['page']  = Ahrefs_Seo::SLUG_SETTINGS;
				$query['tab']   = Ahrefs_Seo_Screen_Settings::TAB_ANALYTICS;
				$query['error'] = $error;
			} else {
				$query['page'] = Ahrefs_Seo::SLUG_CONTENT;
				$query['step'] = 2;
			}
