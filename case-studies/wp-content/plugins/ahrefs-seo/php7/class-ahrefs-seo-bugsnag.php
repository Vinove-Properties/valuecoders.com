	...
	public function report_callback( $report ) {
		try {
			...
			if ( $translation_in_plugin ) {
				Ahrefs_Seo::breadcrumbs( 'Translation error: ' . wp_json_encode( $original_error ) . ' frames: ' . wp_json_encode( $frames ), true );
				return false;
			}
			if ( is_array( $original_error ) && isset( $original_error['message'] ) && strpos( $original_error['message'], 'is marked as crashed and should be repaired' ) ) {
				Ahrefs_Seo::breadcrumbs( 'Table crashed: ' . wp_json_encode( $original_error ), true );
				return false;
			}
