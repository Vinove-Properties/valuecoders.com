...
async function stripe_initialize(){
    let plnPrice    = document.getElementById('op-plan').value;
    let plnType     = document.getElementById('plan-type').value;
    let planCry     = document.getElementById('op-currency').value;
    
    const { id, clientSecret } = await fetch(lp_site_url+"/stripe-payment-init/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ request_type:'create_payment_intent', price:plnPrice, product:plnType, currency:planCry }),
    }).then((r) => r.json());
...
async function handleSubmit(e) {
    ...
    let customer_name   = document.getElementById("op-name").value;
    let customer_email  = document.getElementById("op-email").value;
    
    const { id, customer_id } = await fetch(lp_site_url+"/stripe-payment-init/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
    ...
    if (paymentIntent) {
        ...
        switch (paymentIntent.status) { 
            case "succeeded":
                ...
                fetch(lp_site_url+"/stripe-payment-init/", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ 
                ...
                .then(data => {
                    if(data.payment_txn_id){
                        //console.log( data );
                        window.location.href = lp_site_url+'/stripe-status/?pid='+data.payment_txn_id;
                    }else{
