	...
	public function __toString()
	{
        ...
        switch ($this->license_status) {
            case License::STATUS_EMPTY_LICENSE:
                $license_status_message = esc_html__( "Missing license", 'wp-edd-license-integration' );
                break;
            case License::STATUS_EXPIRED:
                $license_status_message = esc_html__( "Your license key is expired", 'wp-edd-license-integration' );
                break;
            case License::STATUS_REVOKED:
                $license_status_message = esc_html__( "Your license key has been disabled", 'wp-edd-license-integration' );
                break;
            case License::STATUS_MISSING:
            case License::STATUS_INVALID:
                $license_status_message = esc_html__( "Invalid license", 'wp-edd-license-integration' );
                break;
            case License::STATUS_SITE_INACTIVE:
                $license_status_message = esc_html__( "Your license is not active for this URL", 'wp-edd-license-integration' );
                break;
            case License::STATUS_ITEM_NAME_MISMATCH:
                $license_status_message = esc_html__(
            ...
            case License::STATUS_VALID:
				...
				$license_status_message    = esc_html__( "Activated", 'wp-edd-license-integration' );
                break;
            default:
        ...
        $replaces = [
			...
			'{{ lang_status }}'               => esc_html__( 'Status', 'wp-edd-license-integration' ),
			'{{ lang_status_message }}'       => $license_status_message,
            '{{ lang_click_here_read_more }}' => esc_html__(
                'Click here to read more about licenses.',
                'wp-edd-license-integration'
            ),
			'{{ lang_more_info }}'            => esc_html__( 'More information', 'wp-edd-license-integration' ),
        ];
